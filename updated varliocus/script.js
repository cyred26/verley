document.addEventListener('DOMContentLoaded', function () {
    ...
    function updateCart() {
        ...
        if (cartItems.length === 0) {
            orderContainer.innerHTML = "<div class='empty-cart' style='font-size: 18px; padding: 15px 0;'>Your cart is empty</div>";
        } else {
            cartItems.forEach((item, index) => {
                ...
                div.innerHTML = `${item.qty}x ${item.name} - ₱${(item.qty * item.price).toFixed(2)} <button class='remove-btn' data-i='${index}'>x</button>`;
